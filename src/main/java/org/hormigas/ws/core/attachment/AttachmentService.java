...
public class AttachmentService {
    ...
    public enum Status { OK, NOT_FOUND, FORBIDDEN, TOO_LARGE, NOT_UPLOADED, ALREADY_CONFIRMED }

    public record UploadTicket(String attachmentId, String objectKey, PresignedUrl upload) {}

    public record UploadResult(Status status, UploadTicket ticket) {
        static UploadResult fail(Status s) { return new UploadResult(s, null); }
    }

    /** confirm result: on OK, {@code message} must be published into the delivery pipeline. */
    public record ConfirmResult(Status status, Message message) {
        static ConfirmResult of(Status s) { return new ConfirmResult(s, null); }
    }

    public record DownloadResult(Status status, PresignedUrl download) {
        static DownloadResult fail(Status s) { return new DownloadResult(s, null); }
    }
    ...
    public Uni<UploadResult> requestUpload(String conversationId, String uploaderId,
        ...
        if (sizeBytes != null && sizeBytes > maxSizeBytes) {
            return Uni.createFrom().item(UploadResult.fail(Status.TOO_LARGE));
        }
        return conversations.findById(conversationId).flatMap(conv -> {
            if (conv == null) return Uni.createFrom().item(UploadResult.fail(Status.NOT_FOUND));
            if (!conv.hasParticipant(uploaderId)) return Uni.createFrom().item(UploadResult.fail(Status.FORBIDDEN));
            ...
            return attachments.insertPending(pending)
                    ...
                    .map(url -> new UploadResult(Status.OK, new UploadTicket(attachmentId, objectKey, url)));
        });
    }
    ...
    public Uni<ConfirmResult> confirmUpload(String attachmentId, String callerId) {
        return attachments.findById(attachmentId).flatMap(att -> {
            if (att == null) return Uni.createFrom().item(ConfirmResult.of(Status.NOT_FOUND));
            if (!att.uploaderId().equals(callerId)) return Uni.createFrom().item(ConfirmResult.of(Status.FORBIDDEN));
            if (att.status() == AttachmentStatus.CONFIRMED) {
                // Idempotent: a re-confirm does NOT emit a second message.
                return Uni.createFrom().item(ConfirmResult.of(Status.ALREADY_CONFIRMED));
            }
            return storage.exists(att.objectKey()).flatMap(present -> {
                if (!present) return Uni.createFrom().item(ConfirmResult.of(Status.NOT_UPLOADED));
                return attachments.markConfirmed(attachmentId, Instant.now())
                        .replaceWith(conversations.findById(att.conversationId()))
                        .map(conv -> {
                            if (conv == null) return ConfirmResult.of(Status.NOT_FOUND);
                            return new ConfirmResult(Status.OK, toMessage(att, conv));
                        });
            });
        });
    }
    ...
    public Uni<DownloadResult> resolveDownload(String attachmentId, String callerId) {
        return attachments.findById(attachmentId).flatMap(att -> {
            if (att == null || att.status() != AttachmentStatus.CONFIRMED) {
                return Uni.createFrom().item(DownloadResult.fail(Status.NOT_FOUND));
            }
            return conversations.findById(att.conversationId()).flatMap(conv -> {
                if (conv == null) return Uni.createFrom().item(DownloadResult.fail(Status.NOT_FOUND));
                if (!conv.hasParticipant(callerId)) return Uni.createFrom().item(DownloadResult.fail(Status.FORBIDDEN));
                return storage.presignGet(att.objectKey(), downloadTtlSeconds)
                        .map(url -> new DownloadResult(Status.OK, url));
            });
        });
    }
